...
6. `interactiveMode` is set to false. It's a concept associated with a settings.xml file.
    ...
    2. There are [two locations](https://maven.apache.org/settings.html#quick-overview) where a settings.xml file may live: the Maven install: `${maven.home}/conf/settings.xml` or a user's install: `${user.home}/.m2/settings.xml`. I've just checked `${maven.home}/conf/settings.xml` and it's commented out.
